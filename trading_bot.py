...
def add_market_context(d: pd.DataFrame, cfg: Config) -> pd.DataFrame:
    ...
    try:
        ...
        LOG.warning("OI unavailable: %s", e)
    try:
        ...
        LOG.warning("Funding unavailable: %s", e)
    return d
...
def make_features(raw: pd.DataFrame, cfg: Config, with_context: bool = False, labeled: bool = True) -> pd.DataFrame:
    d = raw.copy()
    if with_context:
        d = add_market_context(d, cfg)
    d = _add_indicators(d)
    if labeled:
        d["future_return"] = d.Close.shift(-cfg.horizon_bars) / d.Close - 1
...
def model_pipeline(cfg: Config):
    ...
    if cfg.model == "logistic":
        return Pipeline([("imputer", SimpleImputer(strategy="median")), ("scale", StandardScaler()), ("model", LogisticRegression(C=.5, max_iter=3000, random_state=cfg.seed))])
...
def walk_forward(d: pd.DataFrame, cfg: Config) -> pd.DataFrame:
    ...
    for start in range(cfg.initial_train_bars, len(out), cfg.retrain_every_bars):
        ...
        train, test = out.iloc[:start], out.iloc[start:end]
        if train.label.nunique() < 2:
            continue
        m = model_pipeline(cfg)
        m.fit(train[CORE_FEATURES], train.label)
        out.loc[test.index, "prob_up"] = m.predict_proba(test[CORE_FEATURES])[:, 1]
...
def backtest(d: pd.DataFrame, cfg: Config):
    ...
    for i in range(len(d) - 1):
        ...
        if row.Date.date() != day0:
            day0 = row.Date.date()
            daily_anchor = cash
            halted = False
            trades_today = 0
        dd = cash / max(peak, 1) - 1
        if dd <= -cfg.max_drawdown_pct or (daily_anchor - cash) / max(daily_anchor, 1) >= cfg.max_daily_loss_pct:
            halted = True
        p = float(row.prob_up)
        ...
        if halted or trades_today >= cfg.max_trades_per_day or p < cfg.probability_long or expected <= cost:
            equity.append((row.Date, cash, cash / max(peak, 1) - 1))
            continue
        entry_raw = float(nxt.Open)
        entry = entry_raw * (1 + cfg.slippage_bps / 10000)
        ...
        if lo <= stop:
            ...
            exit_raw, reason = float(nxt.Close), "horizon"
        exit_px = exit_raw * (1 - cfg.slippage_bps / 10000)
        gross = qty * (exit_px - entry)
        fees = (qty * entry + qty * exit_px) * cfg.fee_bps / 10000
        net = gross - fees
        cash += net
        peak = max(peak, cash)
        trades_today += 1
...
def summarize(eq, tr, cfg):
    ...
    sortino = daily.mean() / down * math.sqrt(365) if down and down > 0 else np.nan
    wins = int((tr.net_pnl > 0).sum()) if len(tr) else 0
    gp = float(tr.loc[tr.net_pnl > 0, "net_pnl"].sum()) if wins else 0
    gl = float(-tr.loc[tr.net_pnl <= 0, "net_pnl"].sum()) if len(tr) - wins else 0
...
def latest_signal(cfg: Config):
    """Train only on fully labeled historical bars and score the newest available bar."""
    bars = max(cfg.initial_train_bars + cfg.horizon_bars + 20, 1000)
    raw = download_recent(cfg, bars)
    # Indicators are computed on the complete recent series. Training uses only bars
    # whose future outcome is already known; scoring uses the newest complete feature row.
    train = make_features(raw.iloc[:-cfg.horizon_bars], cfg, with_context=True, labeled=True)
    current = make_features(raw, cfg, with_context=True, labeled=False).iloc[[-1]]
    if len(train) < cfg.initial_train_bars or train.label.nunique() < 2:
        raise RuntimeError("Insufficient labeled history for live signal")
    m = model_pipeline(cfg)
    m.fit(train[CORE_FEATURES], train.label)
    p = float(m.predict_proba(current[CORE_FEATURES])[:, 1][0])
    price = float(current.Close.iloc[0])
    atr = float(current.atr_pct.iloc[0])
    signal = "HOLD" if p < cfg.probability_long else "LONG"
    context = {}
    if "funding_rate" in current:
        context["funding_rate"] = float(current.funding_rate.iloc[0]) if pd.notna(current.funding_rate.iloc[0]) else None
    if "open_interest" in current:
        context["open_interest"] = float(current.open_interest.iloc[0]) if pd.notna(current.open_interest.iloc[0]) else None
...
def main():
    p = argparse.ArgumentParser(description="Research-grade Bitcoin ML trading system")
    p.add_argument("--mode", choices=["backtest", "signal", "data"], default="backtest")
    ...
    if args.mode == "data":
        ...
        report = {"strategy": summarize(eq, tr, cfg), "data_start": str(raw.Date.iloc[0]), "data_end": str(raw.Date.iloc[-1]), "bars": len(raw), "config": asdict(cfg)}
