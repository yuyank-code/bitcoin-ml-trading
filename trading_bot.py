...
def download_recent(cfg: Config, bars: int) -> pd.DataFrame:
    ...
    # Pagination backwards avoids depending on a hard-coded lookback window.
    cursor_end = end
...
def add_market_context(d: pd.DataFrame, cfg: Config) -> pd.DataFrame:
    """Add derivatives context when available. Context is never required for historical training."""
...
def make_features(raw: pd.DataFrame, cfg: Config, with_context: bool = False) -> pd.DataFrame:
    d = raw.copy()
    ...
    d["range_pct"]=(h-l)/c; d["body_pct"]=(c-o)/o
    ...
    d["macd"]=e12-e26; d["macd_signal"]=d.macd.ewm(span=9,adjust=False).mean(); d["macd_hist"]=d.macd-d.macd_signal
    ...
    d["volume_z"]=(v-v.rolling(72).mean())/v.rolling(72).std(); d["volume_ratio"]=v/v.rolling(24).mean()
    d["trend_24_168"]=c.rolling(24).mean()/c.rolling(168).mean()-1; d["drawdown_168"]=c/c.rolling(168).max()-1; d["drawdown_720"]=c/c.rolling(720).max()-1
    d["future_return"]=c.shift(-cfg.horizon_bars)/c-1; d["label"]=(d.future_return>0).astype(int)
    d=d.replace([np.inf,-np.inf],np.nan).dropna(subset=CORE_FEATURES+["future_return","label"]).reset_index(drop=True)
    return d
...
def model_pipeline(cfg: Config):
    ...
    m=HistGradientBoostingClassifier(max_iter=250,learning_rate=.04,max_leaf_nodes=15,min_samples_leaf=30,l2_regularization=1.0,random_state=cfg.seed)
    return Pipeline([("imputer",SimpleImputer(strategy="median")),("model",m)])


def walk_forward(d: pd.DataFrame, cfg: Config) -> pd.DataFrame:
    out=d.copy(); out["prob_up"]=np.nan; out["model_version"]=-1
    version=0
    for start in range(cfg.initial_train_bars,len(out),cfg.retrain_every_bars):
        ...
        m=model_pipeline(cfg); m.fit(train[CORE_FEATURES],train.label)
        out.loc[test.index,"prob_up"]=m.predict_proba(test[CORE_FEATURES])[:,1]; out.loc[test.index,"model_version"]=version; version+=1
    return out.dropna(subset=["prob_up"]).reset_index(drop=True)


def backtest(d: pd.DataFrame, cfg: Config):
    cash=cfg.starting_capital; peak=cash; trades=[]; equity=[]; day0=None; halted=False
    for i in range(len(d)-1):
        row,nxt=d.iloc[i],d.iloc[i+1]
        if row.Date.date()!=day0: day0=row.Date.date(); daily_anchor=cash; halted=False
        ...
        p=float(row.prob_up); expected=float(row.future_return)
        cost=2*(cfg.fee_bps+cfg.slippage_bps)/10000+cfg.min_edge_after_costs
        if halted or p<cfg.probability_long or expected<=cost:
            equity.append((row.Date,cash,cash/max(peak,1)-1)); continue
        entry_raw=float(nxt.Open); entry=entry_raw*(1+cfg.slippage_bps/10000)
        atr=float(row.atr_pct)*entry; stop=entry-cfg.stop_atr_multiple*atr; target=entry+cfg.target_atr_multiple*atr
        qty=min(cash*cfg.risk_per_trade/max(entry-stop,entry*1e-6),cash*cfg.max_position_pct/entry)
        if qty<=0: continue
        ...
        exit_px=exit_raw*(1-cfg.slippage_bps/10000); gross=qty*(exit_px-entry); fees=(qty*entry+qty*exit_px)*cfg.fee_bps/10000; net=gross-fees; cash+=net; peak=max(peak,cash)
    ...
    eq=pd.DataFrame(equity,columns=["Date","capital","drawdown"]).drop_duplicates("Date").sort_values("Date"); tr=pd.DataFrame(trades)
    return eq,tr
...
def latest_signal(cfg: Config):
    # Train on enough recent spot bars; derivatives are used as context/risk diagnostics, not as historical training features.
    bars=max(cfg.initial_train_bars+10,1000); raw=download_recent(cfg,bars); feat=make_features(raw,cfg,with_context=True); train=feat.iloc[:-1]; cur=feat.iloc[[-1]]
    m=model_pipeline(cfg); m.fit(train[CORE_FEATURES],train.label); p=float(m.predict_proba(cur[CORE_FEATURES])[:,1][0]); price=float(cur.Close.iloc[0]); atr=float(cur.atr_pct.iloc[0]);
    signal="HOLD" if p<cfg.probability_long else "LONG"
    context={}
    if "funding_rate" in cur: context["funding_rate"]=float(cur.funding_rate.iloc[0]) if pd.notna(cur.funding_rate.iloc[0]) else None
    if "open_interest" in cur: context["open_interest"]=float(cur.open_interest.iloc[0]) if pd.notna(cur.open_interest.iloc[0]) else None
    return {"timestamp":cur.Date.iloc[0].isoformat(),"symbol":cfg.symbol,"interval":cfg.interval,"price":price,"probability_up":p,"atr_pct":atr,"signal":signal,"context":context,"model":cfg.model}
